...
def calculate_metrics_from_sets(predicted_genes, ground_truth_genes, total_gene_count=20511):
    """Computes precision/recall/F1/MCC metrics between two gene sets."""
    ...
    return tp, fp, fn, precision, recall, f1_score, overlap_str, coverage, mcc
...
def search_optimal_k(mut_df, exp_df, ground_truth, n_patients, n_genes, cancer_name=""):
    ...
    - Primary selection criterion: MCC score; tiebreaker: ground-truth overlap count.
    ...
    cache = {}  # k -> (run_result_tuple, p_value, overlap, mcc)
    ...
    def evaluate_k(k):
        ...
        _, _, _, precision, recall, f1, _, _, mcc = calculate_metrics_from_sets(
            pred_set, gt_set, total_gene_count=n_genes
        )

        cache[k] = (result, p_value, overlap_count, mcc)
        return p_value, overlap_count, mcc

    # Phase 1: coarse search
    PATIENCE = 2 if cancer_name == "Uterine Corpus Endometrial Carcinoma (UCEC)" else 5
    best_k = None
    best_mcc = -2.0  # MCC range is [-1, 1]
    best_overlap_at_best_mcc = -1
    history = {}  # k -> mcc
    ...
    for k in coarse_k_list:
        p_val, overlap, mcc = evaluate_k(k)
        history[k] = mcc
        print(f"    k={k:4d} | p-value={p_val:.5f} | mcc={mcc:.4f} | overlap={overlap}")

        if p_val < 0.01:
            if mcc > best_mcc:
                best_mcc = mcc
                best_overlap_at_best_mcc = overlap
                best_k = k
                no_improve_count = 0
            elif mcc == best_mcc and overlap > best_overlap_at_best_mcc:
                best_overlap_at_best_mcc = overlap
                best_k = k
                no_improve_count = 0
            else:
    ...
    if best_k is None:
        print("  Warning: No k reached p-value < 0.01 in Phase 1. "
              "Falling back to the k with the highest MCC.")
...
def process_single_cancer(cancer_folder):
    res_dir = f"general_results_dynamic_k_mcc_v2/{cancer_folder.name}"
...
def main():
    ...
    if plot_data:
        DISTINCT_COLORS = [
            ...
            '#dcbeff', '#9A6324', "#ffea00", '#800000', '#aaffc3',
        ...
        for idx, (cancer_name, history) in enumerate(plot_data.items()):
            ...
            mcc_vals = [p[1] for p in sorted_pts]

            color = DISTINCT_COLORS[idx % len(DISTINCT_COLORS)]

            ax.plot(k_vals, mcc_vals, linewidth=2.0,
                    label=cancer_name, color=color, alpha=0.85)

        ax.set_xlabel('Number of Factors (k)', labelpad=10)
        ax.set_ylabel('MCC Score', labelpad=10)
        ...
        os.makedirs("general_results_dynamic_k_mcc_v2", exist_ok=True)
        plot_path_png = "general_results_dynamic_k_mcc_v2/elbow_plot_all_cancers_mcc.png"
        plot_path_pdf = "general_results_dynamic_k_mcc_v2/elbow_plot_all_cancers_mcc.pdf"
