...
class GREDCOND:
    ...
    def diversity(self, current_factors, candidate_D):
        ...
        for _, D_i in current_factors:
            ...
            # div(A,B) = 1 - max(|A giao B| / |A|, |A giao B| / |B|) 
    ...
    def fit(self, max_factors=50):
        ...
        while len(self.factors) < max_factors:
            ...
            #print(f"Found Factor {len(self.factors)}: {len(D)} genes, {np.sum(C)} patients. Cumulative Coverage: {coverage_percent:.2f}%")
    ...
    def run_analyze(self, max_factors=50):
        ...
        #print(f"Analyzing {len(self.factors)} factors found by GREDCOND...")

        start_time = time.time()
        
        for idx, (C_mask, D_indices) in enumerate(self.factors):
            # Structural Score (GREDCOND)
